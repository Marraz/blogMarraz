...
namespace blogMarraz.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        public IActionResult DeletePost([FromBody] string postTitle)
        {
            ...
            BlogRepo.Posts.Delete((LiteDB.BsonValue)BlogRepo.Posts.FindOne(p => p.Title == postTitle)._id);
        ...
        public IActionResult UpdatePost([FromBody] Post newPost, [FromBody] string postTitle)
        {
            ...
            if (postId != null)
            {
                bool status = BlogRepo.Posts.Update((LiteDB.BsonValue)postId, newPost);
                if (status)
                {
                    return StatusCode(200);
                }
